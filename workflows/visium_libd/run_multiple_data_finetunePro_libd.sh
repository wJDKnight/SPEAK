...
# Array of data IDs
data_ids=("151508" "151509" "151510")
replicate_name="_rep3R600"  
replicate_name_unconserved="${replicate_name}_unconserved"
fine_tuned_data_name="151507"  # The data used for fine-tuning
...
for data_id in $data_ids; do
    ...
    if [[ "${data_id}" != "${fine_tuned_data_name}" ]]; then
        ...
        python -u workflows/visium_libd/two_in_one_finetunePro_libd.py "${data_id}" "${replicate_name}" "${replicate_name_unconserved}" "${fine_tuned_data_name}" "${skip_first_stage}" > "logs/finetunePro_${fine_tuned_data_name}_${replicate_name}/${data_id}.out" 2>&1 &
    fi
done
