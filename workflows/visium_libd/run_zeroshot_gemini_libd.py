...
# gemini_results_df.index = ['spot_' + str(i) for i in gemini_results_df.index]
# %%
# if the number of the cell type is less than 4, set it to unknown
for nichtype in gemini_results_df.zeroshot_gemini.value_counts()[gemini_results_df.zeroshot_gemini.value_counts()<4].index:
    gemini_results_df.loc[gemini_results_df.zeroshot_gemini == nichtype, "zeroshot_gemini"] = "unknown"
...
print(normalized_mutual_info_score(adata.obs[config.name_truth], adata.obs['zeroshot_gemini_refined']))

#####

adata.obs.to_csv(f"./gemini_results/{config.data_name}_with_refined_{config.model_type}_{config.use_full_name}_{config.with_self_type}_{config.with_region_name}_{config.Graph_type}_{config.with_negatives}_{config.with_CoT}_{config.with_numbers}_{config.with_domain_name}{config.replicate}.csv")
